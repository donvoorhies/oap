        ...
        if (!is_admin() && (!empty($opts['minify']) || !empty($opts['combine']))) {
            ...
            if (!empty($opts['combine'])) {
                ...
                foreach ($wp_styles->queue as $handle) {
                    // Skip admin bar and its dependencies
                    if (in_array($handle, $skip_handles) || in_array($handle, $wp_styles->registered['admin-bar']->deps ?? [])) {
                        continue;
                    }
            ...
            if (!empty($opts['combine'])) {
                ...
                foreach ($wp_scripts->queue as $handle) {
                    // Skip admin bar and its dependencies
                    if (in_array($handle, $skip_handles) || in_array($handle, $wp_scripts->registered['admin-bar']->deps ?? [])) {
                        continue;
                    }
